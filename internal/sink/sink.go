...
// InsertFindings кладёт строки находок. Отдельный батч, но тот же проход по
// потоку: обе таблицы наполняются из одной выборки с шины и подтверждаются
// вместе, поэтому сорванная вставка возвращает на повтор и якоря, и находки.
