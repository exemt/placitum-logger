...
/*
 * parseHeadersFragment разбирает окно JSON-массива пар. Полный объект идёт
 * штатным Unmarshal; обрезанное окно — сканером, чтобы огромный x-load-pad
 * доезжал кусками, а не требовал целиком влезать в один ответ.
 *
 * continuation — кусок значения, у которого в этом окне нет имени: окно
 * началось внутри строки. Карточка дописывает его к последней паре.
 */
